...
import {TimeRangeAccessor} from './TimeRangeAccessor';
...
export class TimeRangeFilter extends SearchkitComponent<RangeFilterProps, any> {
	...
	sliderUpdate(newValues) {
		if ((newValues[0] == this.props.min) && (newValues[1] == this.props.max)){
			...
			this.accessor.state = this.accessor.state.setValue({
				min: this.unixToString(newValues[0]),
				max: this.unixToString(newValues[1])
			});
		}
		this.forceUpdate();
	}
	...
	stringToUnix(str) {
		return moment(str, this.props.format).unix();
	}

	unixToString(unix) {
		return moment.unix(unix).format(this.props.format);
	}
	...
	render() {
		...
		return (
			<div className={classname}>
				...
				{histogram}
				<Rcslider
					min={this.stringToUnix(this.props.min)}
					max={this.stringToUnix(this.props.max)}
					range={true}
					value={[
						this.stringToUnix(get(this.accessor.state.getValue(), "min", this.props.min)),
						this.stringToUnix(get(this.accessor.state.getValue(), "max", this.props.max))
					]}
					tipFormatter={this.unixToString.bind(this)}
